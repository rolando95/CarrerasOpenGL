...
void Colisiones2D::colisionar(int pistaTerreno)
{
	...
	//Calcula el punto m�s cercano al automovil
	...
	//Calcula si el automovil est� colisionando en el plano XZ
	/*
	^ x (y)
	|
	|
	...
	if (pos != 40 && pos != 103 && pos != 111 && pos != 116 && pos != 126){
		...
		if ( coef >= 0) {
			...
			if (h-r < xiz && h+r > xder){//( (parentPos->z - r) < xiz &&  (parentPos->z + r) > xder ) {
				...
				*parentRot = anguloDeg(terreno[pos+1][2] - terreno[pos][2], Vec3(1, 0, 0)) * signoV(terreno[pos+1][2] - terreno[pos][2]).z * -1;
				*parentExtraRot = 0;
			}
		}
		else {
